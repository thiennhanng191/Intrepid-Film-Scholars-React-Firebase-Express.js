...
class OldPost extends Component {
    ...
    handleExpandClick = (event) => {
        ...
        this.props.posts && console.log("post id: ", this.props.post.postId)
    ...
    componentDidUpdate = (prevProps) => {
        ...
        if (this.props.data.post.postId === this.props.post.postId // only update the 1 post just got commented
            && prevProps.data.post.commentCount !== this.props.data.post.commentCount && this.props.data.post.commentCount) {
            if (this.props.data.post.commentCount > this.props.post.commentCount) {
                this.setState({
                    commentCount: this.state.commentCount + 1
                })
            }
            else if (this.props.post.commentCount < prevProps.post.commentCount) {
                this.setState({
                    commentCount: this.state.commentCount - 1
                })
            }
...
*/}
                ...
                <Collapse in={this.state.expanded} timeout='auto' unmountOnExit>
                    <CardContent className={classes.collapseContent}>
                        ...
                        {comments && comments.length === 0 && <div style={{ height: 16 }}></div>}
                        {comments &&
                            comments.length > 0 &&
                            <Fragment>
...
export default connect(mapStateToProps, mapDispatchToProps)(withStyles(styles)(OldPost));
